...
# Open serial port
ser = serial.Serial('COM4', 9600)  # Adjust 'COMx' according to your ESP32-CAM's serial port
...
while True:
    ...
    ret, frame = cap.read()
    if not ret:
        print("Error: Unable to read frame from webcam stream")
        break

    # Preprocess the frame
    resized_frame = cv2.resize(frame, (32, 32))  # Adjust the input size as per your model's requirements
    ...
    if prediction[0][0] > 0.7:  
        ...
        cv2.putText(frame, 'Emergency Vehicle Detected', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
